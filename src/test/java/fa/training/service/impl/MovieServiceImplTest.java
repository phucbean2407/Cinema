...
public class MovieServiceImplTest {
    ...
    private Movie movie;
    private MovieDTO movieDTO, forTest;
...
//    }
    ...
    void findByCategoryTest() {
        ...
        when(movieMapper.castListEntityToDTO(movies)).thenReturn(listMovieCast);
        //Then
        List<MovieDTO> listMovieFound = movieService.findByCategory(category.getName());
        assertEquals(listMovieCast,listMovieFound);
    }
    ...
    void deleteMovieByName() {
        //Given
        MovieDTO expectedDTO = forTest;
        Movie entity = movie;
        Boolean expected = true;
        //When
        when(movieRepository.findByName(expectedDTO.getName())).thenReturn(Optional.of(entity));
        //Then
        Boolean actual = movieService.deleteMovieByName(expectedDTO.getName());
        verify(movieRepository).deleteById(entity.getId());
        assertEquals(expected,actual);
    }
    @Test
    void deleteMovieByNameFalse() {
        MovieDTO expectedDTO = forTest;
        Boolean expected = false;
        //When
        when(movieRepository.findByName(expectedDTO.getName())).thenReturn(Optional.empty());
        //Then
        Boolean actual = movieService.deleteMovieByName(expectedDTO.getName());
        assertEquals(expected,actual);
    }
    ...
    void editMovie() {
        //Given
        MovieDTO expected = forTest;
        Movie entity = movie;
        String expectedString = "Edit Complete";
        //When
        when(movieMapper.castDTOToEntity(expected)).thenReturn(entity);
        when(movieRepository.saveAndFlush(any(Movie.class))).thenReturn(entity);
        //Then
        String actual = movieService.editMovie(expected);
